...
static int parse_zone_info(struct thread_data *td, struct fio_file *f)
{
	...
	for_each_td(td2, i) {
		...
		if ((td2->o.zone_mode==ZONE_MODE_ZBD) && (strcmp(td2->o.filename, f->file_name) == 0))
		...
		for (i = 0; i < nrz; i++, j++, z++, p++) {
			...
			case ZBD_ZONE_COND_EXP_OPEN:
				if (td->o.reset_active_zones_first) {
					...
					p->wp = z->wp;
					p->dev_wp = z->wp;
					dprint(FD_ZBD, "parse_zone_info: open zone = 0x%lX, wp = 0x%lX\n",
							z->start, z->wp);
...
int zbd_setup_files(struct thread_data *td)
{
	...
	unsigned long long total_ow, ow_blks_per_zone, cap_percent;
	...
	for_each_file(td, f, i) {
		...
		dprint(FD_ZBD, "zbd_setup_files: zone finish capacity = 0x%lX, cap_percent = 0x%llX, id = %d, \n",
			  ...
			  struct fio_zone_info *z)
{
	...
	dprint(FD_ZBD, "%s: resetting wp of zone %u, result = %d\n", f->file_name,
		zbd_zone_nr(f->zbd_info, z), result);
			  ...
			  uint32_t zone_idx)
{
	...
	if ((td->o.fill_empty_zones_first) && zbd_zone_full(td, f, z, min_bs))
		return false;
						      ...
						      struct io_u *io_u)
{
	...
	if (find_random && (td->o.max_open_zones || td->o.job_max_open_zones)) {
...
examine_zone:
	if ((z->wp + min_bs <= zbd_zone_capacity_end(td, z)) &&
...
open_other_zone:
    ...
    if ((g_max_open_zones > 0) && (td->o.num_open_zones < td->o.max_open_zones)) {
    	...
    	dprint(FD_ZBD, "%s(%s): open another zone, open zones = %d\n", __func__,
    	       f->file_name, td->o.num_open_zones);
	...
	case DDIR_WRITE:
		...
		if (!zbd_open_zone(td, io_u, zone_idx_b)) {
			if ((zb->cond == ZBD_ZONE_COND_FULL) &&
					td->o.time_based &&
					(full_zones(f) >= (f->zbd_info->nr_zones - g_max_open_zones))) {
				zb->reset_zone = true;
			} else {
				if (!td->o.time_based &&
						(ddir_rw_sum(td->io_bytes) >= (td->zbd_finish_capacity * (f->max_zone - f->min_zone)))) {
					goto eof;
				}
				pthread_mutex_unlock(&zb->mutex);
		 ...
		 * zbd_put_io.
		 */
		if (zb->reset_zone || zbd_zone_full(td, f, zb, min_bs)) {
			...
			if ((td->o.fill_empty_zones_first) &&
				...
				if (full_zones(f) <= (f->zbd_info->nr_zones - g_max_open_zones)) {
					...
					} else if (zbd_zone_full(td, f, zb, min_bs)) {
						zb->reset_zone = true;
					}
				} else {
					if (zbd_fit_in_open_zones_capacity(td, io_u)) {
